import pandas as pd
from sklearn.model_selection import train_test_split
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score
from pgmpy.models import DiscreteBayesianNetwork
from pgmpy.estimators import MaximumLikelihoodEstimator
from pgmpy.inference import VariableElimination

# -----------------------------
# LOAD DATA
# -----------------------------
data = pd.read_csv("C:\\Users\\gmuke\\Desktop\\ML LAB R23\\FINAL MAM\\P7\\7bDiabetes1000.csv")

# Detect target column
target_col = None
for col in ["Outcome", "target", "Class", "Label"]:
    if col in data.columns:
        target_col = col
        break

if target_col is None:
    raise ValueError("Target column not found")

# -----------------------------
# TRAIN-TEST SPLIT (BEFORE DISCRETIZATION)
# -----------------------------
train_data, test_data = train_test_split(
    data,
    test_size=0.25,
    random_state=1,
    stratify=data[target_col]
)

# -----------------------------
# DISCRETIZATION USING TRAIN DATA
# -----------------------------
bin_edges = {}

for col in train_data.columns:
    if col != target_col:
        # Create equal-width bins
        train_data[col], edges = pd.cut(
            train_data[col],
            bins=3,
            labels=False,
            retbins=True
        )
        bin_edges[col] = edges

# Apply same bins to test data
for col in test_data.columns:
    if col != target_col:
        edges = bin_edges[col]
        test_data[col] = pd.cut(
            test_data[col],
            bins=edges,
            labels=False,
            include_lowest=True
        )

# Convert everything to integer
train_data = train_data.fillna(0).astype(int)
test_data = test_data.fillna(0).astype(int)

# -----------------------------
# BUILD BAYESIAN NETWORK
# -----------------------------
features = [c for c in train_data.columns if c != target_col]
edges = [(f, target_col) for f in features]

model = DiscreteBayesianNetwork(edges)

# -----------------------------
# TRAIN MODEL
# -----------------------------
model.fit(train_data, estimator=MaximumLikelihoodEstimator)

# -----------------------------
# INFERENCE
# -----------------------------
infer = VariableElimination(model)

y_true = []
y_pred = []

for _, row in test_data.iterrows():
    evidence = row.drop(target_col).to_dict()
    result = infer.map_query(variables=[target_col], evidence=evidence)

    y_true.append(row[target_col])
    y_pred.append(result[target_col])

# -----------------------------
# EVALUATION
# -----------------------------
print("Bayesian Network Evaluation:")
print("Accuracy :", accuracy_score(y_true, y_pred))
print("Precision:", precision_score(y_true, y_pred, zero_division=0))
print("Recall   :", recall_score(y_true, y_pred, zero_division=0))
print("F1 Score :", f1_score(y_true, y_pred, zero_division=0))

# -----------------------------
# NEW PATIENT PREDICTION
# -----------------------------
sample = test_data.iloc[0]
evidence = sample.drop(target_col).to_dict()

prediction = infer.map_query(
    variables=[target_col],
    evidence=evidence
)

print("\nNew Patient Prediction:",
      "Diabetic" if prediction[target_col] == 1 else "Non-Diabetic")
